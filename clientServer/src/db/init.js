...
import fs from 'fs';
import { DB_PATH, openDb, migratePlaintextConfig, deleteOldRequests } from './index.js';
...
export async function initDatabase() {
  ...
  try {
    await openDb();
    let migrated = 0;
    try { migrated = await migratePlaintextConfig(); }
    catch (err) { console.warn(`[DB] Config migration warning: ${err.message}`); }

    const cutoff = Math.floor(Date.now() / 1000) - CLEANUP_SECONDS;
    let filesDeleted = 0, rowsDeleted = 0;
    try {
      const { getRecentRequests } = await import('./index.js');
      const oldRequests = await getRecentRequests(10000);
      for (const req of oldRequests) {
        if (req.createdAt < cutoff) {
          if (req.reqBodyPath && fs.existsSync(req.reqBodyPath)) { try { fs.unlinkSync(req.reqBodyPath); filesDeleted++; } catch {} }
          if (req.resBodyPath && fs.existsSync(req.resBodyPath)) { try { fs.unlinkSync(req.resBodyPath); filesDeleted++; } catch {} }
        }
      }
    ...
    return { created: !dbExists, cleaned: rowsDeleted, filesDeleted, migrated };
  } catch (err) {
